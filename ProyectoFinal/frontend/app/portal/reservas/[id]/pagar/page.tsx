...
export default function PagarReservaPage() {
  ...
  const handlePagar = async () => {
    ...
    try {
      ...
      setExito(resultado)
      if (metodo === 'EFECTIVO') {
        showSuccess('Reserva registrada! Recuerda pagar al llegar al local.')
      } else {
  ...
  if (exito) {
    const esPresencial = exito.comprobante.estadoPago === 'PENDIENTE'
        ...
        <div className="container mx-auto px-4 py-12 pt-24">
          <div className="max-w-lg mx-auto">
            <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
              <div className={`w-20 h-20 ${esPresencial ? 'bg-orange-500' : 'bg-green-500'} rounded-full flex items-center justify-center mx-auto mb-6`}>
                {esPresencial ? (
              ...
              <h1 className="text-2xl font-bold text-slate-800 mb-2">
                {esPresencial ? 'Reserva Registrada!' : 'Pago Exitoso!'}
              </h1>
              <p className="text-slate-500 mb-8">
                {esPresencial
                  ? 'Tu reserva ha sido registrada. Paga al llegar.'
              ...
              <div className="bg-slate-50 rounded-xl p-6 text-left mb-6">
                <h3 className="font-semibold text-slate-800 mb-4 flex items-center gap-2">
                  ...
                  {esPresencial ? 'Comprobante de Reserva' : 'Comprobante de Pago'}
                </h3>
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between py-2 border-b border-slate-200">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      esPresencial ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'
                    }`}>
                      {esPresencial ? 'Pago Pendiente' : 'Pagado'}
                    </span>
                  </div>
                  <div className="flex justify-between py-3">
                    <span className="text-slate-500">{esPresencial ? 'Monto a pagar' : 'Monto pagado'}</span>
                    <span className={`font-bold text-xl ${esPresencial ? 'text-orange-600' : 'text-green-600'}`}>
                      ${exito.comprobante.monto}
                    </span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="text-sm text-amber-800">
                      ...
                      <ul className="space-y-1 text-amber-700">
                        <li>• Llega 15 minutos antes de tu horario</li>
                        <li>• Presenta este codigo de referencia</li>
                        <li>• El pago se confirma al llegar al local</li>
                      </ul>
                    </div>
                  </div>
                </div>
