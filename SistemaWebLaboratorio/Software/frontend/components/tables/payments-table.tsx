...
export function PaymentsTable({ items, onPay, onReceipt }: Props){
  const toStatus = (estado: PaymentItem['estado']): StatusKind => (estado === 'PAGADO' ? 'pagado' : estado === 'VENCIDO' ? 'vencido' : 'pendiente');
  return (
          ...
          <tr key={f.numero_factura} className="align-top">
            ...
            <td className="border-b border-border-soft p-2">
              <div className="flex gap-2">
                {f.estado !== 'PAGADO' && (
                  <button className="icon" aria-label="Pagar" onClick={() => onPay?.(f.numero_factura)}>Pagar</button>
                )}
                {f.estado === 'PAGADO' && (
                  <button className="icon" aria-label="Descargar comprobante" onClick={() => onReceipt?.(f.numero_factura)}>Descargar</button>
                )}
              </div>
            </td>
          </tr>
